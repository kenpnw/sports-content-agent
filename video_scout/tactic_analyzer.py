...
class VideoScoutAnalyzer:
    ...
    def _base_system_prompt(self) -> str:
        return (
            ...
            "**严格禁止以下情况强行套战术名**（这些不是战术，是结果/事件/状态，硬套会让分析显得幼稚）：\n"
            "・所有失误（turnover/bad pass/lost ball/out of bounds/travel/violation）→ 这是失败案例，写"失误回合，原因是 XX"，不写"属于 XX 战术"；\n"
            "・节末/比赛结束（period end / game end）→ 这是时刻不是战术，写"Q1/Q2/Q3/Q4 收尾"或"全场结束"；\n"
            "・个人单打 ISO 中持球人随意运球后强攻 → 这是个人能力不是战术；\n"
            "・抢板二次进攻仅"抢板→直接补攻"，没有跑位配合 → 写"二次进攻"即可，不必战术化；\n"
            "・罚球得分/罚球未中 → 写"罚球时刻"；\n"
            "・无明显战术配合的过渡进攻、转换快攻 → 写"快攻 / Transition"即可，不必硬套阵型名。\n\n"
            "正确示例：\n"
            "・命中且有战术 → \"本回合属于「一五挡拆」(1-5 PnR) 战术。Dort 上提为 SGA 做掩护，迫使湖人换防...\"\n"
            "・失误 → \"本回合是 L. James 的传球失误。他在弧顶持球面对包夹，将球传向空切的浓眉时被 Caruso 提前预判抢断。\"\n"
            "・节末 → \"Q2 收尾时刻。雷霆领先 8 分进入下半场，最后一次进攻选择不冒险出手，让时间走完。\""
        )
