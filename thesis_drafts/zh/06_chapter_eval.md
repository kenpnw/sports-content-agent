...
为证明本系统不仅适用于 OKC vs LAL G1 这一单场比赛，本研究在[第二场比赛占位]上完整跑通了端到端流水线。结果汇总如下：

- **PBP 接入**：成功，约 [N] 个事件；
- **OCR 时间映射**：[N] 个有效样本，覆盖全部 4 节；
- **可见性检测**：播放段占比 [N]%，与人工标注吻合；
- **5-Agent 输出**：60 个 segment，事实正确率 [N]%（来自事实判官评估）；
- **多平台打包**：4 个平台 package 全部成功生成。

该场比赛的转播商为 [Network 占位]，与 OKC vs LAL G1 的转播商不同。auto_roi_detector 模块成功标定了大致的记分牌 ROI 区域（IoU 约 [N]%），最终通过人工微调收敛到精确位置，标定耗时约 5 分钟。这一结果表明本系统的核心组件（OCR + 可见性检测 + 5-Agent）在跨比赛、跨转播商场景下具备基本的泛化能力，工程改进点主要集中在 ROI 自动标定的精度。
